...
1. 应用改造支持双写：全量迁移期间及之后，主动同步所有新写操作（增删改）
	...
	```
2. 然后全量数据迁移：使用数据迁移工具（如 ShardingSphere Migration、DataX）从原库导出数据，按新分片规则将数据导入新库。应该用的MVCC保证一致性视图
...
#### 4.2.2.3 辅助索引表
##### 4.2.2.3.1 设计前提和核心定位
例如实例系统实例表，按时间范围分库分表，但用户想要查询“运行中”的实例。全量扫描耗时太长，那么可以建立一张“运行中实例索引表”。
它是一张独立的 “轻量表”，仅存储 “当前处于运行中状态的实例” 的核心信息。这个索引表的设计要遵循「简洁、低侵入、最终一致」原则。
**最终一致性**：允许短暂延迟（比如实例状态从 “运行中”→“欠费关停”，索引表 1 秒内同步），实例系统 QPS 不高，延迟无感知。

##### 4.2.2.3.2 分表设计
如果是供用户查询，那么可以按照用户哈希分表（数据量不大也可以不分表）。

##### 4.2.2.3.3 数据同步流程（触发时机 + 同步方式）
索引表的数据同步完全依赖 “实例状态变更事件”，触发时机和同步方式如下：
1. 只有当实例状态发生 “运行中↔非运行中” 切换时，才会同步索引表。运行中->非运行中时，新增索引；非运行中->运行中时，删除索引。
2. 如果实例表和索引表在一个库中，那可以使用本地事务；否则可以用消息队列更新索引表。

#### 4.2.2.4 生命周期长的数据怎么处理
例如实例系统分库分表之后，有些实例可能生命周期很长，长达几年，导致一些老库（如instance_db_2022）一直被占用。这种情况要怎么处理呢？
处理的核心思路是 **“分类迁移 + 渐进式归档 + 老库退役”**：区分 “仍在使用的老实例” 和 “已闲置的老实例”，分别处理后释放老库资源，同时保障业务无感知、数据不丢失。

##### 4.2.2.4.1 界定 “可归档老实例” 与 “需保留老实例”
通过这一步，可将老库中的实例 “分流”：大部分闲置实例归档，少量在用实例迁移到新库，为老库退役铺路。
1. 可归档老实例（核心迁移对象）：状态非运行中且近 1 年无状态变更、创建时间≥3 年（可按业务调整，如 2 年 / 5 年）的，可迁移到归档存储
2. 需保留老实例（暂时不迁移）：状态运行中或或近 1 年有状态变更

##### 4.2.2.4.2 TODO：具体处理方案（分两类实例落地）
迁移流程（渐进式，业务无感知）：
1. 准备工作：
	- 新建归档库：`instance_archive_db`（1 主 1 从，低配服务器，如 2 核 4G，满足查询需求即可）；
	- 表结构同步：复制老库的表结构（实例表、快照表）到归档库，保留索引（确保查询效率）；
	- 数据备份：迁移前备份老库数据（全量备份 + binlog），避免数据丢失。
2. 数据迁移（全量 + 增量，低峰期执行）
	- 全量迁移：用 ShardingSphere-Migration 或 MySQLdump，将老库中 “可归档实例” 的全量数据迁移到归档库（如`instance_archive_db.t_instance_2022`、`instance_archive_db.t_snapshot_2022`）；
	    - 注意：仅迁移 “可归档实例”，避免迁移需保留的实例；
	- 增量同步：通过 binlog 同步工具（如 Canal），同步老库中 “可归档实例” 的增量数据（如迁移期间的状态变更），确保数据一致性；
	- 校验：迁移完成后，随机抽查 100 条数据（对比老库和归档库的实例信息、快照记录），确保无遗漏、无错误。
3. 路由切换（中间件层修改，无感知）：分库分表中间件（如 ShardingSphere-JDBC）添加路由规则：
    - 当查询实例号为 “2022xx-xxx-xxxx”（2022 年创建）且属于 “可归档实例” 时，自动路由到`instance_archive_db`；
    - 保留临时路由：若归档库查询异常，可快速回滚路由。
...
# 6 分库分表之后，一些系统里面的小表怎么放呢？
 最佳方案是**单独建立 “公共库”（也叫 “基础库 / 元数据库”），让元数据表常驻其中，不随业务库（按时间分库的实例表、快照表）迁移**，既解决 “迁移麻烦” 的问题，又保障查询效率。

# 7 面试题：为什么系统设计没考虑分库分表
## 7.1 面试官的考察重点
