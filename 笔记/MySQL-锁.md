...
InnoDB行锁是通过索引上的索引项来实现的，只有通过索引条件检索数据，InnoDB才会使用行级锁，否则，InnoDB将使用表锁。索引分为主键索引和非主键索引两种，如果一条sql语句操作了主键索引，MySQL就会锁定这条主键索引；如果一条语句操作了非主键索引，MySQL会先锁定该非主键索引，再锁定相关的主键索引。
...
之所以要这样做是为了保证事务的隔离性和原子性。假如不是两阶段，那么事务A的多个更新、和事务B的对相同数据的多个更新可能会**交错执行**，那么A出现问题回滚的时候，就会把B的更新也回滚了。

**如果你的事务中需要锁多个行，要把最可能造成锁冲突、最可能影响并发度的锁尽量往后放**。

### 3.1.1 读已提交的锁优化
在读已提交隔离级别下还有一个优化，即：**语句执行过程中加上的行锁，在语句执行完成后，就要把“不满足条件的行”上的行锁直接释放了，不需要等到事务提交**。
...
- 行锁
	- InnoDB 支持行锁的，MyISAM不支持
	- 行锁是通过锁索引实现的，如果操作了非主键索引，会先锁定该非主键索引，再锁定相关的主键索引。
	- 两阶段锁协议：行锁在需要的时候加上，等到事务结束时才释放。
		- 这是为了事务的隔离性和原子性，防止两个事务交错更新时、其中一个回滚导致数据不一致
		- 在事务中，锁操作尽量往后放
		- 读已提交隔离级别的行锁优化：语句执行过程中加上的行锁，在执行完后就会把“不满足条件的行”的行锁释放，不需要等到事务提交
	- 出现死锁后，有两种策略
		- 直接进入等待，直到超时。超时时间 `innodb_lock_wait_timeout` 默认是 50s
		- 死锁检测：发现死锁后，主动回滚死锁链条中的某一个事务，让其他事务得以继续执行。死锁检测会消耗大量的 CPU 资源，每个新来的被阻塞的线程，都要判断会不会由于自己的加入导致了死锁，这是一个时间复杂度是 O(n^2) 的操作。
- 幻读和间隙锁
	- 幻读会导致数据和binlog(statement格式)产生不一致。
		- 例如表有a、b两个字段（a有索引），有数据 (0,0)
		- 事务A把a=0的b更新为5
		- 然后事务B插入一条 (0,1) 的数据，提交事务。
		- 最后A提交事务。此时数据 (0,5), (0,1)
		- binlog 的sql（statement格式）如下
		```sql
		insert t values(0,1)
		update t set b=5 where a=0
		
		// 数据 (0,5),(0,5)，从库和主库不一致
		```
	- 间隙锁：产生幻读的原因是，行锁只能锁住行，但是新插入记录这个动作，要更新的是记录之间的“间隙。InnoDB 只好引入间隙锁 (Gap Lock)，锁住记录之间的间隙。
		- 间隙锁之间都不存在冲突关系。跟间隙锁存在冲突关系的，是“往这个间隙中插入一个记录”这个操作。
		- 间隙锁和行锁合称 next-key lock，每个 next-key lock 是前开后闭区间
		- 间隙锁会导致语句锁住更大的范围，影响了并发度并且可能导致死锁。
			- 例如表有id字段，数据 (0), (5)
			- 事务A用select for update查询id=3的数据
			- 然后事务B也用select for update查询id=3的数据，因为间隙锁不冲突，此时B不阻塞
			- 然后事务B尝试插入id=3的数据，被A的间隙锁锁住
			- 然后事务A尝试插入id=3的数据，被B的间隙锁锁住。出现死锁
		- 读已提交隔离级别的大部分操作没有间隙锁（除了唯一键约束检查等操作），配合上row格式的binlog，能增加并发读，避免binlog不一致和间隙锁的死锁问题
	- **TODO**：间隙锁详解
- **TODO**：怎么排查死锁：`show engine innodb status` 的输出中有一节 `LATESTDETECTED DEADLOCK`，就是记录的最后一次死锁信息。它的`TRANSACTIONS` 一节也给出了事务在等待什么锁。
- 意向锁：在加行锁前，先给表加 “意向锁”，声明 “我要对表中的某行加锁”。后续表级锁请求只需检查 “表上是否有意向锁”，无需遍历行。意向锁也分为读锁（`SELECT FOR SHARE`）、写锁。
- 悲观锁和乐观锁：
	- 悲观锁：`SELECT ... FOR UPDATE`，也可加 `SKIP LOCKED`实现非阻塞悲观锁。适用于短事务。
	- 乐观锁：通过版本号字段控制并发。适用于并发冲突频率较低的场景，或不希望长时间持有锁的场景。
		- 注意乐观锁版本号更新成功并不意味着锁住了资源，正确用法需要配合 “操作前再次校验状态”，确保当前处理的前提条件仍成立。
