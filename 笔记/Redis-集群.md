...
## 1.5 总结
- 流程：
	- 全量同步：主库先生成自己的RDB，传给从库
	- 增量同步：主库把全量同步期间的写操作，写入一个 replication buffer（每个从库一个）。全量同步完成后，把里面的内容发给从库
- 重连后增量同步：
	- repl_backlog_buffer 是一个环形缓冲区，所有从库通用。锻炼期间的写操作都记录在里面
	- 当从库重连后，给主库发送自己的读下标。如果被主库的写下标覆盖了，那就只能再次全量同步；否则主库把两者之间的命令增量同步给从库。
...
## 2.5 总结
- 哨兵机制实现主从库自动切换，哨兵其实就是一个运行在特殊模式下的 Redis 进程。哨兵之间通过 pub/sub 机制互相发现，并获得从库地址
- 主从切换流程：
	- 监控：
		- 主观下线：哨兵给主库、从库发送 PING 命令，如果响应超时会把它标记为主观下线
		- 客观下线：标记主库主观下线的哨兵，会给其他哨兵发消息确认，如果有 >=`quorum` 配置项个哨兵都标记主库主观下线，那就标记主库为客观下线
	- 哨兵 Leader 选举：哨兵 Leader 负责实际的主从切换
		- 标记客观下线的哨兵，向其他哨兵发起投票，让自己作为主从切换的 Leader
		- 如果拿到 >=`quorum` 配置项的票，则当上 Leader；否则随机等待一段时间再重新投票
	- 新主库筛选：
		- 先把和主库断连过多次的从库剃掉
		- 某个从库优先级配置项最高，则选它，否则继续往下
		- 某个从库和主库同步进度最接近，则选它，否则继续往下
		- 最后选择编号最小的从库
	- 通知：
		- 哨兵把新主库的连接信息发给其他从库，让它们和新主库建立连接并进行数据复制。
		- 同时，哨兵把新主库的连接信息通知给客户端，让它们把请求操作发到新主库上。
...
## 3.3 总结
- Redis Cluster，又叫切片集群。一个切片集群共有 16384（$2^{14}$） 个哈希槽，哈希槽类似于数据分区，键值对会根据它的 key，被哈希映射到一个哈希槽中。Redis 会自动把这些槽平均分布在集群实例上，也可以手动分配。
- 哈希槽信息传递：
	- Redis 实例会把自己的哈希槽信息发给和它相连接的其它实例，来完成哈希槽分配信息的扩散，类似于Gossip分布式共识算法
	- 客户端和集群实例建立连接后，实例就会把哈希槽的分配信息发给客户端，客户端缓存下来。
- 重定向机制：集群有实例上下线，或者负载均衡，都会引起哈希槽的移动。实例可以相互发消息更新信息，客户端需要重定向机制
	- 如果实例发现没有客户端请求的哈希槽，那么会返回`MOVED` 响应结果，包含新实例的访问地址。客户端会请求新实例并更新缓存信息
	- 如果实例发现哈希槽移了一半，没有客户端请求的数据，那么会返回`ASK`响应结果，包含新实例的访问地址。客户端会向新实例发送 `ASKING` 命令然后请求数据，但不会更新缓存信息。
