...
### 2.2.6 sync.WaitGroup 使用不当（Add/Done 不匹配）
...
### 2.2.7 select 语句无 default 分支，且所有 case 均无法触发
例如：
...
### 2.2.9 Timer 泄漏（time.After 使用不当）
...
### 2.2.10 IO 操作无超时（网络、数据库、Redis 等）
...
- 协程泄漏的原因主要有
	...
	- 锁竞争激烈：锁的力度太大，或者锁的时间很长，导致大量协程阻塞
		...
		- 如果读比写操作多很多，可以用读写锁
		- 用原子操作或无锁数据结构替代锁
		- 协程池限流（兜底方案）：若锁竞争无法避免，那需要做好限流，避免协程一直增长
	- 无限循环无退出条件，导致协程一直在运行
...
## 3.1 第一步：确认 CPU 暴涨的现象（先搞清楚 “是什么问题”）
1. **看监控面板（优先）**：
...
- 首先需要确认变慢的范围：
	...
	- 慢的情况是缓慢增长还是突增：缓慢增长，那可能是内存、协程泄漏的问题；突增，那可能是流量大、依赖或线上变更问题
- 通过定位慢的范围，然后根据情况先看下程序问题或依赖问题
	- 系统资源：
		...
		- 网络带宽耗尽：大流量请求、文件上传下载无限流。
	- 程序问题：
		- 如果近期有代码或配置变更，那就优先看下改动的部分是不是有问题
		- 然后可以用 pprof 定位看下CPU耗时和协程状态，定位慢的逻辑，是否有协程泄漏、锁竞争激烈、低效循环、大量数据处理等问题
		- 同时看下日志，筛选慢的请求，排查其中每个步骤的耗时，看是哪里慢
	- 依赖问题
		- 数据库：查看有没有慢查询、数据库连接池有没有被打满、数据库资源占用情况怎么样
