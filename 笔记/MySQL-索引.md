...
那么：
- **第 1 层（根节点）**：1160 个节点（B + 树初始根节点为叶子节点，随数据量增长升级为非叶子节点，此处按非叶子节点计算）。
- **第 2 层**：由根节点的索引项数量决定，共 1160×1160=1,345,600 个节点。
- **第 3 层**：每个节点对应上层 1 个索引项，因此节点数 = 1160×1160×1160=1,560,896,000 个。
- **第 4 层（叶子节点）**：每个节点对应第 3 层的 1 个索引项，因此节点数 = 1160³×15=23,413,440,000 ≈ 234亿。

如果是三层，那么能存 2018 万条数据。
...
- 索引是在引擎层实现的，InnoDB的索引是B+树结构
	- B+树是多叉树，只有叶子结点存行数据，非叶子节点只存索引；而B树非叶子节点也存储数据。所以一般 B+ 树的叶节点和非叶节点大小不同，而 B 树则相同；并且 B+ 树的查询效率固定为 O(logN)，B树最好能有 O(1)。
	- B+树的叶子结点之间有指针链接，从而串成一个循环双向链表。
	- B+每个非叶子节点能容纳更多 key，能索引的范围更大，因此树高也更低，减少磁盘IO。并且双向链表还增强了区间访问性，利好范围查询
- InnoDB每个索引都是一颗B+树，称为索引组织表。其中主键索引的叶子结点是一行数据，又叫聚簇索引；其他索引的叶子结点是主键，又叫二级索引。使用二级索引查数据时，还需要再回到主键索引查询真实的行，称为回表。
- B+树有一些常见的面试题：
	- 一页能存多少数据：主键（8字节），一行（1K）
		- 去掉页头、页尾、页目录，一页（16K）大概还剩16200字节
		- 节点指针大小6B
		- 因此非叶子结点能存 16200/(8+6) ≈1160
		- 叶子结点能存 16200/1K ≈ 15
	- B+树能存多少数据：主键（8字节），一行（1K）
		- 3层能存 2000万 ≈ 1160²×15
		- 4层能存 234亿 ≈ 1160³×15
	- B+树怎么优化
		- 优化索引结构，减少索引大小（字符串前缀索引）
		- 控制索引数量，去掉无用索引
		- 提高页利用率：使用有序索引防止页分裂；定期整理数据页，页合并
		- 硬件与配置优化：使用固态硬盘；调大MySQL缓冲池
		- 分表分库
...
- 索引常见失效情况：index dive、不符合最左前缀、隐式类型转换、字段用函数表达式包围
- 走不走索引：
	- or 和索引合并相关
