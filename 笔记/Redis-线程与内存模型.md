...
- 所有客户端的命令请求，都会进入一个 “**命令队列**”，Redis 主线程会依次从队列中取出命令执行，同一时间只会执行一个命令，不会有多个命令并行执行；
- 而 Redis 会把整个 Lua 脚本，视为「一个独立的、不可拆分的命令」。执行期间 “禁止中断”，要么全成要么全败
...
# 3 过期删除
Redis 的过期删除并非依赖单一策略，而是通过 **「惰性删除 + 定期删除 + 内存淘汰机制」** 的组合实现，核心目标是在「CPU 开销」和「内存占用」之间找到平衡 —— 既避免频繁扫描导致的性能损耗，又防止过期 key 长期堆积造成内存浪费。

## 3.1 核心策略 1：惰性删除（Lazy Eviction）
Redis 不会主动监控 key 是否过期，而是在 **用户主动访问某个 key 时**（如 `GET`、`MGET`、`HGET` 等读操作），才会触发过期检查：
1. 先查询该 key 的过期时间（**Redis 用独立的「过期字典」存储 key 与过期时间的映射，不与数据字典混淆**）；
2. 若 key 已过期，立即从数据字典和过期字典中删除该 key；
3. 返回 `nil` 给客户端（或不返回该 key 的数据）。

## 3.2 核心策略 2：定期删除（Periodic Eviction）
为解决惰性删除的内存堆积问题，Redis 会定期（默认每 100ms）执行一次 “过期 key 清理任务”，流程如下：
1. **随机抽样**：每次执行时，从「过期字典」中随机选取 `N` 个 key（`N` 默认为 20，可通过 `maxmemory-samples` 配置调整）；
2. **删除过期 key**：检查这 `N` 个 key 的过期时间，删除其中已过期的 key；
3. **循环条件**：如果本次抽样中，过期 key 的比例超过 25%，则重复步骤 1-2（继续抽样删除）；若比例≤25%，则结束本次定期任务；
4. **时间限制**：为避免长时间阻塞主线程，每次定期任务的执行时间**严格限制在 25ms 以内**（与 Redis 事件循环的节拍 `hz` 相关，`hz` 默认 10，即每秒 10 次循环，每次循环最多分配 100ms/10=10ms。实际源码中是通过 `server.hz` 动态调整，但核心是 “不阻塞主线程”）。

关键配置：
- `hz`：Redis 事件循环的频率（默认 10，范围 1-500），`hz` 越大，定期任务执行越频繁，过期 key 清理越快，但 CPU 开销越高；
- `maxmemory-samples`：每次抽样的 key 数量（默认 20），样本越多，清理越彻底，但单次任务耗时可能增加。

## 3.3 核心策略 3：内存淘汰机制（Maxmemory Eviction）
你可以使用下面这个命令来设定缓存的大小：
```bash
$ CONFIG SET maxmemory 4gb
```
当 Redis 占用的内存达到配置的 `maxmemory` 阈值时，会触发 “内存淘汰机制”—— 此时无论 key 是否过期，都会按预设策略淘汰部分 key，确保内存不溢出。

### 3.3.1 基础知识
Redis 4.0 之前一共实现了 6 种内存淘汰策略，在 4.0 之后，又增加了 2 种策略。
不进行数据淘汰的策略，只有 noeviction 这一种，它会拒绝所有写操作（返回 OOM 错误）。
...
## 3.4 总结
- Redis 的过期删除是由三个方面共同实现的：惰性删除、定期删除、和内存淘汰
- 惰性删除：用户访问key时，发现它过期了，就把它删除并返回nil
- 定期删除：会有一个后台线程每隔一段时间，从过期字典中随机选取N个键值对，把里面过期的删除掉。如果删除的比例>25%，那么就重复抽样删除的步骤。
- 内存淘汰：当Redis内存占用要超过设置的最大值时，会根据配置的内存淘汰策略清理内存，给新值腾位置。
	- 内存淘汰策略有8中，可以分成三类：无内存淘汰、所有键值对淘汰、过期键值对淘汰。
		- 无内存淘汰就是不淘汰内存，返回OOM错误。
		- 所有键值对淘汰就是在所有键值对中，挑选一部分淘汰。它又分成随机淘汰、LRU淘汰、LFU淘汰三种策略。
		- 过期键值对淘汰就是在设置了过期时间的键值对中，挑选一部分淘汰。它又分成随机淘汰、LRU淘汰、LFU淘汰、和过期时间淘汰四种策略。
	- LRU淘汰原理：Redis会先随机挑选一些键值对组成LRU链表，淘汰其中最早被访问的键值对；当再次淘汰时，会再挑选一些访问时间早于链表中最早访问时间的键值对补充道链表中，在进行淘汰。
		- 这样避免了维护所有键值对的链表，降低了性能开销
	- LFU淘汰原理：LFU相比于LRU多了访问次数的统计，所以它会把访问次数最小&&访问时间最早的键值对淘汰。这样能有效应对扫描式查询操作
		- LFU的访问次数是非线性递增的方法，因此只需要8bit存访问次数，也能有效地区分不同的访问次数
		- LFU的访问次数还会随着最近两次访问时间的间隔衰减，把那些较早之前被大量访问的键值对尽早淘汰出去
