...
- 维度：
	...
	- 接口维度：单个or所有
	- 机器维度：单个or所有
	- 时间维度：突变还是持续变化；一直存在还是随着QPS、流量起伏
	- 报错维度：是否有报错，报错数量大吗
	- 下游依赖维度：数据库、中间件、其他服务
...
## 3.1 第一步：确认现象与数据收集
### 3.1.1 确认现象：先验证是否真泄漏
- **量化协程数量**：看协程数量是激增还是持续增长。激增可能是流量大了，或用户做了什么并发量大的复杂操作；持续增长则很可能是泄漏
- **关联系统状态**：观察 CPU、内存、GC 情况：
    - 若协程增长很多但 CPU 使用率低：大概率是协程阻塞（如 channel、WaitGroup）；
...
## 3.5 总结
- 首先需要确认是否协程泄漏：协程激增可能是流量大了，或用户做了什么并发量大的复杂操作；持续增长则很可能是泄漏
- 泄露的大致原因：如果协程数很高但是CPU不高，那么可能是有很多协程被阻塞了；如果协程数很高并且CPU也很高，那么可能是协程在低效循环或复杂计算
...
# 4 CPU暴涨问题排查
线上 CPU 使用率暴涨是 Go 服务常见的性能问题，排查核心是**先定位 “CPU 高耗的进程 / 线程”，再锁定 “具体耗时的代码逻辑”**。

## 4.1 第一步：确认 CPU 暴涨的现象（先搞清楚 “是什么问题”）
首先看：
- 看看QPS是否升高
- 有没有哪个接口响应变慢：它里面会做比较重的操作吗（复杂计算or并发量大）
- 机器维度：单个or所有
- 时间维度：突变还是持续变化
- 报错维度：是否有报错，报错数量大吗
...
- 首先要确认CPU暴涨的现象
	- 看看QPS是否升高
	- 有没有哪个接口响应变慢：它里面会做比较重的操作吗（复杂计算or并发量大）
	- 机器维度：单机器占用高，说明可能是机器配置问题；多机器占用高，说明可能是流量大、代码逻辑问题
	- 时间维度：突变还是持续变化。突变可能是QPS突增或用户做了重操作；持续变化可能是QPS持续增长或者协程泄漏
...
- 原因：
	...
	- 原子操作多，竞争激烈：导致自旋频繁
	- 协程泄漏：过多导致调度开销大
	- GC频繁：短生命周期对象频繁创建、大内存对象频繁分配

# 5 内存泄漏问题排查
## 5.1 先判断是不是真泄漏
在排查前，首先要明确服务的内存增长是**正常的业务波动**还是**真的内存泄漏**，避免做无用功。
关键判断依据：
1. **监控指标观察**：
    - 关注 Go 的核心内存指标：`heap_inuse`（当前堆内存使用量）、`heap_alloc`（堆内存已分配量）。
    - 如果这些指标**随着服务运行时间持续上涨**，即使请求量稳定或下降，也没有回落（GC 后内存不释放），基本可以判定是内存泄漏。
2. **复现条件**：
    - 重启服务后，内存会快速下降，然后再次缓慢 / 快速上涨，呈现 “重启就好，过段时间又卡” 的规律。

## 5.2 开启 pprof 并采集数据
参见《pprof》一节。
通过`go tool pprof`工具分析采集到的 profile 文件，重点关注**堆内存**和**协程**两个维度，这是 Go 服务内存泄漏的主要来源。
协程泄漏是 Go 中**最常见的内存泄漏原因**（每个 goroutine 默认占用 2KB 左右的栈内存，大量协程阻塞会导致内存暴涨）。

## 5.3 常见原因
1. 协程泄露
2. 全局变量或长生命周期对象的不当使用：全局变量（如 map、切片）或长生命周期对象（如单例）中存储了不再使用的数据，且未及时清理
3. 资源未关闭
...
## 5.4 总结
- 先确认是不是内存泄露：内存随着时间一直增长，并且请求量稳定或下降也没回落，那么大概率是内存泄漏；重启服务后，还是会这样，那基本可以确定
- 排查：使用pprof，从堆内存和协程两个方面排查
- 常见原因：
	- 协程泄漏
	- 长生命周期对象，持有不用的数据也没清理
	- 资源未关闭
	- 切片截取引发的泄露：底层数组很大，切片较小且一直存在
...
| ---- | ------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------- |
| 接口维度 | 单个接口 vs 全量接口慢？<br><br>耗时分位值（P50/P90/P99）是否均高？     | 监控面板（Prometheus<br>+Grafana）、<br>日志聚合（ELK/ Loki） | 若只有单个接口慢→聚焦该接口的逻辑 / 依赖 / QPS / 数据量；<br><br>若全量慢→系统资源 / 全局依赖问题 |
...
- 然后需要确认变慢的范围：
	- 单个或几个接口慢还是所有接口慢：
		- 前者可能是QPS、请求数据量、接口内复杂操作、代码逻辑或依赖问题
		- 后者可能是QPS或核心依赖（数据库）问题
	- 有没有错误：如果报错很多，那可能是依赖问题
	- 慢的情况是缓慢增长还是突增：缓慢增长，那可能是内存、协程泄漏的问题；突增，那可能是QPS高、请求数据量大、依赖或线上变更问题
