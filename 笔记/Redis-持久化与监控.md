...
1. **Always**，同步写回：每个写命令执行完，立马同步地将日志写回磁盘，也就是调用 fsync()，然后才向客户端返回 “执行成功”。
2. **Everysec**，每秒写回：每个写命令执行完，只是先把日志写到 AOF 文件的内存缓冲区，也就是调用 write()；后台每隔一秒调用 fsync()；
3. **No**，操作系统控制的写回：每个写命令执行完，只是先调用 write()，由操作系统决定何时调用 fsync()
...
## 1.3 AOF always 能保证完全不丢数据吗？
`always` 能保证 “命令成功响应后不丢”，但无法覆盖以下极端场景：
1. 磁盘写缓存未关闭：操作系统和物理磁盘都有 “写缓存”，`fsync` 的作用是 “把操作系统页缓存（page cache）的数据刷到磁盘”，但如果磁盘本身开启了 “写回模式”（Write Back，默认很多磁盘会开启）
2. 主从复制异步导致的集群级数据丢失：
	- 主节点执行写命令，`fsync` 到 AOF 后，向客户端返回成功；
	- 但主节点还没来得及把该命令同步到从节点（主从复制是异步的，有微小延迟）；
	- 此时主节点突然宕机（比如硬件故障），哨兵触发主从切换，将从节点提升为新主节点；
	- 新主节点上没有该条未同步的命令，导致数据丢失（客户端已收到成功响应，但集群切换后数据不见了）。
3. 极端硬件 / 系统故障（不可抗因素）
	- 物理磁盘损坏（比如磁盘磁道坏道、闪存损坏）：AOF 文件本身被破坏，即使 `fsync` 成功，数据也可能丢失；
	- 服务器主板、CPU 故障：导致 `fsync` 过程中断，虽然概率极低，但可能造成 AOF 日志写入不完整；

极端场景的解决办法：
1. 关闭磁盘写缓存
2. 主从复制 + 同步确认：开启 “主节点写命令需同步到从节点后才返回成功”，避免异步复制的延迟丢失：
	- 主节点设置 `min-replicas-to-write 1`（至少有 1 个从节点在线）；
	- 主节点设置 `min-replicas-max-lag 10`（从节点延迟不超过 10 秒）；
	- 开启 `repl-diskless-sync no`（从节点同步数据时，先写磁盘再返回确认，避免从节点内存数据丢失）。
3. AOF 日志校验 + 集群冗余
	- AOF 日志校验：开启 `aof-checksum yes`（Redis 6.2+ 支持），Redis 重启时会校验 AOF 日志的完整性，避免因日志损坏导致数据丢失；
	- 集群冗余：使用 Redis Cluster（至少 3 主 3 从），每个主节点有多个从节点，即使一个从节点数据丢失，还有其他从节点可用；
	- 定期备份：结合 AOF 日志和 RDB 快照（比如每天凌晨执行一次 RDB 备份），即使 AOF 日志损坏，也能通过 RDB 恢复到最近备份点，再通过 AOF 日志回放增量数据。

## 1.4 总结
- AOF是写后日志，记录Redis命令。它在命令之后同步写，会影响性能。有三种写回策略：Always、Everysec、No
	- Always 不能完全防止丢数据，磁盘写缓存开启、主从复制不一致、硬件损坏 这些问题都会导致丢数据
- AOF重写
	- AOF 在 文件大小超过最小重写大小 & 文件增量比例超过最小重写比例 时，会触发AOF重写，把内存最新的数据存储成命令
	- 后台使用 fork 子进程、写时复制机制重写 AOF：
		- 先同步拷贝父进程内存页表（内存太大会阻塞），子进程后台把内存数据写入重写文件
		- 当Redis修改数据时，父进程申请新的内存页（修改bigkey会阻塞），同时把命令写入重写缓冲区
		- 重写文件写完，合并重写文件和重写缓冲区
		- 最后父进程同步替换当前的AOF文件
- RDB 记录的是某一时刻的内存快照，并不是操作。
	- 每隔一段时间，也是后台使用 fork 子进程、写时复制机制生成RDB
- 混合使用：每隔一段时间，生成RDB；两次快照之间，使用 AOF 记录所有命令。这样避免了AOF重写，数据恢复时，使用RDB+AOF也很快。
	- 单独用RDB，不好控制频率
	- 单独用AOF，重写有开销，而且AOF数据恢复速度不如RDB
- 如何选择：
	- 如果可以接受短时间内的数据丢失，可以只启用RDB。
	- 如果要尽量保证数据不丢失，可以混合使用。
