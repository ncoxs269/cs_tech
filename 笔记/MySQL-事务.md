...
## 2.6 可重复读能避免幻读问题吗？
可重复读隔离级别下，有MVCC防止快照读的幻读问题，next-key锁防止当前读的幻读问题，看起来似乎能完全避免幻读了。
但幻读仍可能发生的关键原因是：**快照读和当前读的行为不一致**。
如果同一事务内先执行快照读，再执行当前读，且两次读取之间有其他事务插入并提交了新记录，当前读会读到新记录，导致 “幻读”。
...
INSERT INTO `user` VALUES (1, '张三'), (3, '王五');
```
场景步骤：
1. **事务 A（RR 级别）启动**，执行快照读（普通查询），查询`id > 1`的记录：
	```mysql
	BEGIN; -- 事务A启动
	-- 第一次读：快照读，基于事务A启动时的快照
	SELECT * FROM `user` WHERE id > 1; 
	-- 结果：(3, '王五')（此时无其他记录）
	```
2. **事务 B 启动**，插入一条新记录并提交：
    ```sql
    BEGIN; -- 事务B启动
    INSERT INTO `user` VALUES (2, '李四'); -- 插入id=2（满足id>1）
    COMMIT; -- 事务B提交，新记录生效
    ```
3. **事务 A** 再次执行查询，但这次使用当前读（加锁读）：
    ```sql
    -- 第二次读：当前读（加锁），读取最新数据
    SELECT * FROM `user` WHERE id > 1 FOR UPDATE;
    -- 结果：(2, '李四'), (3, '王五')（出现了第一次未读到的新记录）
    ```

因此，当同一事务内混合使用快照读和当前读，且中间有其他事务插入新记录时，仍可能出现幻读。若要完全防止幻读，需使用最高隔离级别**串行化（Serializable）**，但会牺牲并发性能。
...
- MVVC的实现机制：undo log、版本链、Read View
	...
	- Read View：
		- 读已提交的每次快照读、可重复读的第一条SQL，都会开启一个Read View，它用来做可见性判断。
...
- 可重复读也不能完全避免幻读问题，当同一事务内混合使用快照读和当前读，且中间有其他事务插入并提交新记录时，当前读仍会读到新纪录，出现幻读。得靠串行化完全解决。
...
只要事务提交，redo log就保证写入，即使崩溃也能通过redo log恢复数据。
## 4.2 双写缓冲区
**双写缓冲区解决 “数据页刷盘部分写问题”**：若数据页刷盘时崩溃（如写了一半），会导致数据页损坏，且 redo log 无法修复（因为 redo log 基于完整数据页）。
原理：数据页刷盘前，先复制到内存中的 “双写缓冲区”，再分两步写入磁盘：
1. 先将双写缓冲区的内容写入磁盘的 “双写文件”（连续空间，顺序写，快）；
2. 再将数据页从双写缓冲区写入实际数据文件。若崩溃，可从双写文件恢复完整数据页，再用 redo log 修复。

写入双写文件时崩溃，双写文件可能出现 “部分写入”，但 InnoDB 有完整的校验和与恢复机制，不会导致数据丢失或损坏。
### 4.2.1 先明确：双写文件的本质是 “数据页的临时备份区”
首先要纠正一个认知：双写文件不是 “最终存储”，而是「数据页写入实际数据文件（.ibd 文件）前的 “完整备份”」。它的核心作用是：**当实际数据页写入崩溃时，提供一个 “干净的完整版本” 用于恢复**；而双写文件本身的 “部分写入”，可以通过其他机制修复。

InnoDB 中双写文件的设计细节：
1. **存储位置**：默认放在 InnoDB 共享表空间（ibdata1）中，也可通过 `innodb_doublewrite_file` 配置独立文件；
2. **空间大小**：固定为 2MB × 2 = 4MB（两个 2MB 的连续块，称为 “doublewrite buffer chunks”），循环使用；
3. **数据格式**：存储的是「完整的数据页（默认 16KB）」，每个数据页会附带校验信息（校验和、页号、LSN 日志序列号）。

### 4.2.2 关键：写入双写文件的 “原子性保障” 与 “崩溃容忍”
双写文件的写入是 **“按块原子化”** 的，且每个数据页带校验和，崩溃后能识别并丢弃损坏的部分。
双写缓冲区（内存中的一块区域，默认 2MB）中的数据，写入磁盘双写文件时，会分两步执行，且都是「顺序 IO」（效率极高，避免随机写的延迟）：
1. 写入双写文件的“chunk1”或“chunk2”（连续块） 
2. 调用 fsync 刷盘（确保数据写入物理磁盘，而非操作系统缓存）

这里的关键是：磁盘的最小读写单位是「**扇区**」（默认 512 字节，现代 SSD 多为 4KB），InnoDB 会将双写文件的数据页对齐到磁盘扇区。
这意味着：一个 16KB 的数据页写入双写文件时，会拆分成 4 个 4KB 的扇区写入，**单个扇区的写入是原子的（要么全成功，要么全失败）**，不会出现 “一个扇区写了一半” 的情况。

InnoDB 启动时，会执行「崩溃恢复流程」，其中专门针对双写文件的校验与恢复步骤如下：
1. 校验双写文件的每个数据页，检查每个数据页的「校验和」「页号一致性」「LSN 连续性」：
	- 若校验通过（数据页完整）：说明该数据页已成功写入双写文件，后续可用于修复实际数据文件；
	- 若校验失败（数据页部分写入）：说明该数据页是 “损坏块”，直接丢弃（因为双写文件是备份，损坏的备份毫无意义，且 redo log 中已有完整的修改记录）。
2. 分场景恢复数据：
	- 场景 A：写入双写文件时崩溃（双写文件有损坏块），丢弃双写文件中的损坏块，直接使用「磁盘上的原始数据页」（未被修改的旧版本），然后通过 redo log（事务提交时已刷盘）重新执行修改，恢复到事务提交后的状态。
	- 场景 B：写入实际数据文件时崩溃（双写文件已写完整），实际数据页可能部分写入（损坏），此时 InnoDB 会用双写文件中「校验通过的完整数据页」覆盖损坏的实际数据页，然后再通过 redo log 重放后续修改，确保数据完整。
3. 若双写文件写入成功，但未调用 fsync（数据在 OS 缓存），但「原始数据页未被修改」，依然可以通过 redo log 重放修改，恢复数据。
### 4.2.3 关键补充：双写缓冲区的 “性能影响” 与 “设计取舍”
你可能会好奇：“多写一次双写文件，会不会变慢？”——InnoDB 通过两个设计抵消性能损耗：
1. 双写文件是「连续存储 + 顺序写」：顺序写的效率远高于随机写（数据文件的写入是随机写，因为数据页分散在表空间中），所以写入双写文件的额外开销很小；
2. 双写缓冲区是「批量写入」：InnoDB 会积累一定量的数据页后，一次性写入双写文件，减少 IO 次数。
