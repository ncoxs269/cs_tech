...
如果两张都是大表，且存在热点 Key，需要给热点 Key 加盐，将大表拆分成多个小表做 join，再合并结果。
**步骤（以 product_id 为热点 Key 为例）**：
1. **对热点 Key 加盐**：给大表 A 的热点 product_id 添加 0~9 的随机前缀，给大表 B 的热点 product_id 也添加 0~9 的随机前缀；
2. **对非热点 Key 加盐**：给大表 A 和 B 的非热点 product_id 添加固定前缀（比如 0），避免影响正常数据；
3. **join 操作**：两张表按加盐后的 product_id join；
4. **合并结果**：得到最终的 join 结果。

代码：
```sql
-- 原SQL：大表join大表，product_id=10086导致数据倾斜
SELECT 
    o.order_id,
    o.product_id,
    c.cost,
...
FROM 
    order_table o
JOIN 
    cost_table c ON o.product_id = c.product_id;


-- 优化后的SQL：大表join大表的热点Key加盐处理
WITH 
-- 第一步：处理订单大表，给product_id加盐
order_table_salt AS (
    SELECT 
        order_id,
        product_id,
        order_amount,
        order_time,
        -- 对热点Key（10086）添加0~9的随机前缀，非热点Key添加固定前缀0
        CASE 
            WHEN product_id = 10086 THEN CONCAT(CAST(RAND() * 10 AS INT), '_', product_id)
            ELSE CONCAT('0_', product_id) 
        END AS salt_product_id  -- 加盐后的关联键
    FROM order_table
),
-- 第二步：处理成本大表，给product_id加盐（与订单表的加盐规则完全一致！）
cost_table_salt AS (
    SELECT 
        product_id,
        cost,
        cost_time,
        -- 必须和订单表用相同的加盐规则：热点Key加随机前缀，非热点Key加固定前缀0
        CASE 
            WHEN product_id = 10086 THEN CONCAT(CAST(RAND() * 10 AS INT), '_', product_id)
            ELSE CONCAT('0_', product_id) 
        END AS salt_product_id  -- 加盐后的关联键
...
-- 第三步：用加盐后的关联键进行join，此时热点Key的数据已被分散
join_result AS (
    SELECT 
        o.order_id,
        o.product_id,  -- 保留原product_id，不影响业务使用
        c.cost,
        o.order_amount,
        o.order_time
